    ...
    if readfile and isfile then
        pcall(function()
            if isfile(animDataCachePath) then
                ...
                if type(data) == "table" then diskCache = data end
            end
        end)
    end
end
...
local function getAnimDataForBundle(bundleId, bundled)
    ...
    if _animDataCache[cacheKey] then return _animDataCache[cacheKey] end
...
--[[
    FIX #1: Clone + Destroy instead of reparenting.
    Reparenting keeps the old Lua VM state alive inside the Animate script,
    which can corrupt the movement state machine. Clone+Destroy creates a
    completely fresh instance every time.
    
    FIX #2: Force the Humanoid's Running state off then on.
    This "kicks" the Animate script's state machine so it properly
    re-evaluates whether the player is walking, fixing the "can jump
    but can't walk" bug.
]]
local function restartAnimate(char)
    ...
    -- Clone + Destroy = completely fresh script, no stale state
    ...
    -- Force the humanoid movement state to re-evaluate
...
--[[
    FIX #3: Much more thorough un-stick function.
    Resets WalkSpeed, un-anchors, clears bad states, forces Running
    state re-evaluation, and restarts Animate fresh.
]]
local function ensureCharacterCanMove(char)
    ...
    -- Restart Animate fresh as a last resort
    local animate = char:FindFirstChild("Animate")
    if animate then
        local clone = animate:Clone()
        animate:Destroy()
...
local function applyAnim(data)
    ...
    task.spawn(function()
        ...
        notify("Animation", "Loading: " .. tostring(data.name or "Animation") .. "...", 8)
        task.wait(0.2)
...
local function applySlotFromBundle(slotName, bundleData)
    ...
    local myChar = char; local myGen = applyGeneration
    ...
    for sn, anims in pairs(allData) do
        if sn:lower() == slotName:lower() then targetAnims = anims; break end
    end
...
local SortList = Instance.new("UIListLayout"); SortList.Padding = UDim.new(0, 0.02, 0); SortList.HorizontalAlignment = Enum.HorizontalAlignment.Center; SortList.VerticalAlignment = Enum.VerticalAlignment.Top; SortList.SortOrder = Enum.SortOrder.LayoutOrder; SortList.Parent = SortFrame
...
local function createsort(order, text, sort)
    local CreatedSort = Instance.new("TextButton"); CreatedSort.Size = UDim2.new(0.94, 0, 0, 28); CreatedSort.BackgroundColor3 = Color3.fromRGB(30, 30, 30); CreatedSort.LayoutOrder = order; CreatedSort.TextColor3 = Color3.new(1, 1, 1); CreatedSort.Text = text; CreatedSort.Font = Enum.Font.GothamBold; CreatedSort.TextSize = 13; CreatedSort.BorderSizePixel = 0; CreatedSort.ZIndex = 101; Corner:Clone().Parent = CreatedSort; CreatedSort.Parent = SortFrame
...
local function ensurePoolSize(count)
    while #buttonPool < count do
        ...
        local Favorite = Instance.new("ImageButton"); Favorite.Name = "favorite"; Favorite.Image = FavoriteOff; Favorite.AnchorPoint = Vector2.new(0.5, 0.5); Favorite.Size = UDim2.new(0.32, 0, 0.32, 0); Favorite.Position = UDim2.new(0.88, 0, 0.88, 0); Favorite.BorderSizePixel = 0; Favorite.BackgroundTransparency = 1; Favorite.Parent = btn
        ...
        Favorite.MouseButton1Click:Connect(function() local item = btnDataMap[btn]; if item then toggleFav(item.data.id, item.data.name, item.data.bundledItems) end end)
...
local function getItemsPerPage()
    ...
    if absX > 0 and absY > 0 then local cols = math.floor((absX - 8) / 80); local rows = math.floor(absY / 80); return math.max(1, math.max(1, cols) * math.max(1, rows)) end
    return 60
end
...
function refreshGrid()
    ...
    for i = 1, #buttonPool do
        ...
        if itemIdx <= endIdx and combined[itemIdx] then
            ...
            if State.mode == "animation" then btn.Image = "rbxthumb://type=BundleThumbnail&id=" .. item.data.id .. "&w=420&h=420" else btn.Image = "rbxthumb://type=Asset&id=" .. item.data.id .. "&w=150&h=150" end
            btn:FindFirstChild("favorite").Image = item.isFav and FavoriteOn or FavoriteOff
        else btn.Visible = false; btnDataMap[btn] = nil end
    end
end

Frame:GetPropertyChangedSignal("AbsoluteSize"):Connect(function() local newIPP = getItemsPerPage(); if newIPP ~= State.itemsPerPage then State.itemsPerPage = newIPP; if ScreenGui.Enabled then refreshGrid() end end end)

PageLeft.MouseButton1Click:Connect(function() local totalPages = math.max(1, math.ceil(#State.cachedCombined / State.itemsPerPage)); if totalPages <= 1 then return end; if State.currentPage > 1 then State.currentPage = State.currentPage - 1 else State.currentPage = totalPages end; refreshGrid() end)
PageRight.MouseButton1Click:Connect(function() local totalPages = math.max(1, math.ceil(#State.cachedCombined / State.itemsPerPage)); if totalPages <= 1 then return end; if State.currentPage < totalPages then State.currentPage = State.currentPage + 1 else State.currentPage = 1 end; refreshGrid() end)
...
PageLabel.FocusLost:Connect(function(enterPressed) local text = PageLabel.Text:gsub("%s", ""); local pageNum = tonumber(text); if pageNum then local totalPages = math.max(1, math.ceil(#State.cachedCombined / State.itemsPerPage)); State.currentPage = math.clamp(math.floor(pageNum), 1, totalPages) end; refreshGrid() end)
...
local function openCustomAnimEditor()
    ...
    if not customFrame then
        ...
        clearAllBtn.MouseButton1Click:Connect(function() State.config.CustomAnimSlots = {}; SaveConfig(); captureOriginalAnims(); revertAllSlotsSafely(); for _, child in pairs(scrollFrame:GetChildren()) do if child:IsA("Frame") then local cl = child:FindFirstChild("CurrentLabel"); if cl then cl.Text = "None" end end end end)
    end
    for _, slotName in ipairs(ANIM_SLOT_NAMES) do
        local row = customFrame:FindFirstChild("Row_" .. slotName, true)
        if row then local cl = row:FindFirstChild("CurrentLabel"); if cl then local currentInfo = State.config.CustomAnimSlots[slotName]; cl.Text = (type(currentInfo) == "table" and currentInfo.name) and currentInfo.name or "None" end end
    end
...
SearchBar:GetPropertyChangedSignal("Text"):Connect(function() local text = SearchBar.Text; if text ~= text:sub(1, 50) then SearchBar.Text = text:sub(1, 50); text = SearchBar.Text end; searchItems(text); refreshGrid() end)
...
AutoButton.MouseButton1Click:Connect(function() State.autoReapplyEnabled = not State.autoReapplyEnabled; State.config.AutoReapplyEnabled = State.autoReapplyEnabled; SaveConfig(); AutoButton.Text = State.autoReapplyEnabled and "RE: ON" or "RE"; notify("Auto-Reapply", State.autoReapplyEnabled and "ON" or "OFF", 3); if State.autoReapplyEnabled then task.spawn(function() local hasCustomSlots = State.config.CustomAnimSlots and next(State.config.CustomAnimSlots); if getgenv().lastAnim and not hasCustomSlots then applyAnim(getgenv().lastAnim) elseif hasCustomSlots then applyAllCustomSlots() end end) end end)
    ...
    if ScreenGui.Enabled then
        ...
        inputconnect = UserInputService.InputBegan:Connect(function(input, processed)
            if not processed and input.UserInputType == Enum.UserInputType.MouseButton1 then
                local mousePos = UserInputService.GetMouseLocation and UserInputService:GetMouseLocation() or input.Position; local framePos = BackFrame.AbsolutePosition; local frameSize = BackFrame.AbsoluteSize
                if mousePos.X < framePos.X or mousePos.X > framePos.X + frameSize.X or mousePos.Y < framePos.Y or mousePos.Y > framePos.Y + frameSize.Y then ScreenGui.Enabled = false end
            end
        end)
    else
        if customFrame then customFrame.Visible = false; BackFrame.Visible = true end; if inputconnect then inputconnect:Disconnect() end
    end
end)
...
--[[
    FIX #4: onCharacterAdded now has stuck detection.
    After auto-reapply finishes, we wait 5 seconds and check if the
    player has moved. If they haven't, we force-unstick them.
]]
local function onCharacterAdded(char)
    ...
    if State.autoReapplyEnabled then
        ...
        task.wait(2) -- Slightly longer wait to ensure character is fully initialized
        ...
        -- Safety net: wait for apply to finish, then ensure character can move
        task.spawn(function()
            local elapsed = 0
        ...
        -- FIX #4: Stuck detection - if player hasn't moved after 6 seconds, force unstick
        task.spawn(function()
            ...
            if (currentPos - startPos).Magnitude < 2 then
                ensureCharacterCanMove(char)
                notify("Fix", "Force-unstuck your character", 3)
            end
        end)
    end
...
task.spawn(function()
    LoadConfig(); loadLastAnim(); loadDiskCache()
